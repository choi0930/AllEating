...
<body class="d-flex flex-column min-vh-100">
	<form action="${contextPath}/admin/addCoupon.do" name="frmCoupon" method="post" enctype="UTF-8">
	<div class="areaCoupon">
		<div class="couponTitleText">쿠폰 생성</div>
		<div class="couponAreaBar"></div>
		<div class="couponArea">
			<div>쿠폰 이름</div>
			<div class="couponName"><input type="text"  class="form-control" name="coupon_name" placeholder="45자 이내로 작성해주세요"/></div>
			<div></div>
		</div>
		<div class="couponArea">
			<div>쿠폰 할인율</div>
			<div><input type="text"  class="form-control" name="couponDiscountRate" id="discountInput" maxlength="2" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');"/>%</div>
			<div>쿠폰 적용 범위</div>
			<div>
		...
		<div class="couponArea">
			<div>배포시작일</div>
			<div><input type="date" class="form-control" name="beginDate" value="${dateMap.beginDate}"></div>
			<div>배포종료일</div>
			<div><input type="date" class="form-control" name="endDate" value="${dateMap.endDate}"></div>
		</div>
		<div class="couponArea">
			<div>쿠폰 사용시작일</div>
			<div><input type="date" class="form-control" name="coupon_startDate" value="${dateMap.beginDate}"></div>
			<div>쿠폰 사용종료일</div>
			<div><input type="date" class="form-control" name="coupon_endDate" value="${dateMap.endDate}"></div>
		</div>
		<div class="couponBtn">
			<div class="btn">
				<button type="button" class="btn btn-outline-primary" onclick="fn_addCoupon()">쿠폰 생성</button>
			</div>
			<div class="btn">
